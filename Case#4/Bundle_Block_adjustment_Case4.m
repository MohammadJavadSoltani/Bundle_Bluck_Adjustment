%% Project 4 --> MohammadJavadSoltani
#Calc:EOP : wieghted and observed by GPS and INS +...
#Coods(Tie+ Z for horizantal GCPs + X,Y for Vertical GCPs)
#Just 4 GCP considered and rest are tie points.

close all; clear; clc; format long g; tic;
%% First We Load Our Parameters Coumputed In Last Project And Our Data
load('parameters1.mat'); load('parameters2.mat');
load('data.mat'); load('data_control_points.mat');
...
plot(compelete_control_point(:, 2), compelete_control_point(:, 3), '*');
text(compelete_control_point(:, 2), compelete_control_point(:, 3), int2str(compelete_control_point(:, 1)));
C = 153.692 / 1000; % mm -> m
% So 1109 - 2189 - 1709 - 2789 Are Our Full Control points.
%% 4 FULL Cp. 
block_control_IDs = [1109; 2189; 1709; 2789];
block_control_points = zeros(4, 4);%4 CP / ID + XYZ
for i = 1 : length(data_control_points)
   for j = 1 : length(block_control_IDs)
       if data_control_points(i, 1) == block_control_IDs(j)
           block_control_points(j, :) = data_control_points(i, :);
       end
   end
end%block_control_points Are satisfied!
block_control_points = sortrows(block_control_points, 1);
block_control_points(:, 5) = [1:4];% Numbering them
block_control_points_copy = block_control_points;%Creat a Copy From it.
...
result_images(:, 8) = result_images(:, 6);%Use from prj1 result
result_images(:, 6 : 7) = zeros(14, 2);
result_images_copy = result_images;%Creat a Copy From it.
...
Fg_H = [jacobian(Fx, [X, Y]); jacobian(Fy, [X, Y])];
Fg_Plan = [jacobian(Fx, Z); jacobian(Fy, Z)];
%% P Matrix
VARIANCE = [eye(328) * 7 * 10 ^ -6, zeros(328, 96); zeros(12, 328), eye(12) * 0.15, zeros(12, 84)];
for i=1:8%sigma for Z is diffrent.
    VARIANCE(len_data * 2 + 3*i,len_data * 2 + 3*i) = (20 / 100) .^ 2;
    if i<2
        VARIANCE(len_data * 2 + 3*8 + 2*2 +1) = (20 / 100) .^ 2;
    end
end
%Now for EOPs:
for i = 1 : 14 % 0.2 ->XY   /   0.25 -> Z   / Omega,phi,kappa ->5''
    VARIANCE(6 * i - 5 + 340 : 6 * i + 340, 6 * i - 5 + 340 : 6 * i + 340) = [0.2, 0, 0, 0, 0, 0; 0, 0.2, 0, 0, 0, 0; 0, 0, 0.25, 0, 0, 0; 0, 0, 0, 5 / 3600 / 180 * pi, 0, 0; 0, 0, 0, 0, 5 / 3600 / 180 * pi, 0; 0, 0, 0, 0, 0, 5 / 3600 / 180 * pi];
end
VARIANCE = VARIANCE .^ 2;
P = inv(VARIANCE);
...
while norm(delta_x) > 10e-12
    ...
    for i = 1 : len_data
        xcheck = false;
        for k = 1 : length(block_control_points)
           if data(i, 3) == block_control_points(k) || data(i, 6) == 0%Tie or Full Controll
              xcheck = true;
           end
        end
        if ~xcheck
...
%             num = data(i, 7) - 100;
 ...
 %       those 4 Full CPs are done here.
%        elseif data(i, 6) == 2
%             num  = data(i, 7) - 1000;
%             X = plan_control_point(num, 2);
%             Y = plan_control_point(num, 3);
%             Z = plan_control_point(num, 4);
%             Ag(2 * i_temp - 1 : 2 * i_temp, 50 * 3 + 2 + num) = [(2768668935719305*(sin(Kappa)*sin(Omega) + cos(Kappa)*cos(Omega)*sin(Phi)))/(18014398509481984*(cos(Omega)*cos(Phi)*(Z - Z0) - sin(Phi)*(X - X0) + cos(Phi)*sin(Omega)*(Y - Y0))) - (2768668935719305*cos(Omega)*cos(Phi)*((Z - Z0)*(sin(Kappa)*sin(Omega) + cos(Kappa)*cos(Omega)*sin(Phi)) - (Y - Y0)*(cos(Omega)*sin(Kappa) - cos(Kappa)*sin(Omega)*sin(Phi)) + cos(Kappa)*cos(Phi)*(X - X0)))/(18014398509481984*(cos(Omega)*cos(Phi)*(Z - Z0) - sin(Phi)*(X - X0) + cos(Phi)*sin(Omega)*(Y - Y0))^2);
...
%  
        ...
        w(2 * i_temp - 1, 1) = x + (2768668935719305*((Z - Z0)*(sin(Kappa)*sin(Omega) + cos(Kappa)*cos(Omega)*sin(Phi)) - (Y - Y0)*(cos(Omega)*sin(Kappa) - cos(Kappa)*sin(Omega)*sin(Phi)) + cos(Kappa)*cos(Phi)*(X - X0)))/(18014398509481984*(cos(Omega)*cos(Phi)*(Z - Z0) - sin(Phi)*(X - X0) + cos(Phi)*sin(Omega)*(Y - Y0)));
 
        w(2 * i_temp, 1) = y + (2768668935719305*((Y - Y0)*(cos(Kappa)*cos(Omega) + sin(Kappa)*sin(Omega)*sin(Phi)) - (Z - Z0)*(cos(Kappa)*sin(Omega) - cos(Omega)*sin(Kappa)*sin(Phi)) + cos(Phi)*sin(Kappa)*(X - X0)))/(18014398509481984*(cos(Omega)*cos(Phi)*(Z - Z0) - sin(Phi)*(X - X0) + cos(Phi)*sin(Omega)*(Y - Y0)));
 
    end
    A = [AEO, Ag];
    %-I3*3 For 4 Full CPs.
    for i = 1 : 4
        A(328 + 3 * i - 2 : 328 + 3 * i, 14 * 6 + 50 * 3 + 3 * i - 2 : 14 * 6 + 50 * 3 + 3 * i) = -eye(3);
    end
    %-I6*6 For EOPs
    for i = 1 : 14
       A(340 + 6 * i - 5 : 340 + 6 * i, 6 * i - 5 : 6 * i) = -eye(6); 
    end
    ...
    for i = 1 : 4%Full
        block_control_points_copy(i, 2 : 4) = delta_x(14 * 6 + 50 * 3 + 3 * i - 2 : 3 * i + 14 * 6 + 50 * 3, 1)' + block_control_points_copy(i, 2 : 4);
    end
    %Update W
    for i = 1 : 4
       w(328 + 3 * i - 2 : 328 + 3 * i, 1) = [block_control_points(i, 2) - block_control_points_copy(i, 2); block_control_points(i, 3) - block_control_points_copy(i, 3); block_control_points(i, 4) - block_control_points_copy(i, 4)];
    end
    
    for i = 1 : 14
       w(340 + 6 * i - 5 : 340 + 6 * i, 1) = -(result_images(i, 3 : 8)' - result_images_copy(i, 3 : 8)');
    end
    norm(delta_x);
...
for i = 1 : 4
   X_Computed(i, 1) = block_control_points_copy(i, 2);
   Y_Computed(i, 1) = block_control_points_copy(i, 3);
   Z_Computed(i, 1) = block_control_points_copy(i, 4);
end
figure();
plot(X_Computed, Y_Computed, '*r', 'MarkerSize', 10);%Computed Control Points.
hold on
plot(data_control_points(:, 2), data_control_points(:, 3), '*g');%Origin Control Points.
text(data_control_points(:, 2), data_control_points(:, 3), int2str(data_control_points(:, 1)));
legend('Control Points Computed', 'Control Points');

for i = 1 : 4
   X_Main(i, 1) = block_control_points(i, 2);
   Y_Main(i, 1) = block_control_points(i, 3);
   Z_Main(i, 1) = block_control_points(i, 4);
end

Main_Coordinates = table(X_Main, Y_Main, Z_Main)
Computed_Coordinates = table(X_Computed, Y_Computed, Z_Computed)

Ran = result_images(:, 1);
Image_Num = result_images(:, 2);
X0 = result_images(:, 3);
Y0 = result_images(:, 4);
Z0 = result_images(:, 5);
Omega = result_images(:, 6);
Phi = result_images(:, 7);
Kappa = result_images(:, 8);

Exterior_Parameters = table(Ran, Image_Num, X0, Y0, Z0, Omega, Phi, Kappa)
